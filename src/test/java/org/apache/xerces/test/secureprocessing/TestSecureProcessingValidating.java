 ...
 * 
 */
public class TestSecureProcessingValidating extends TestSecureProcessing
{
    @Override
    ...
    void doTestExternalEntities(ParserType type, EntityType entityType, E systemProperty, E secureProcessing, E externalParameterEntities, String expected) throws Exception
    {
        ...
        else
        {
            ...
            catch (SAXParseException e)
            {
                ...
                Assert.assertTrue(e.getLocalizedMessage().indexOf("Element type \"externalParameterEntity\" must be declared.") > -1);
            }
            catch (Exception e)
            {
                e.printStackTrace();
    ...
    void doTestExternalDTD(ParserType type, E systemProperty, E secureProcessing, E loadExternalDTD, String systemProtocols, String factoryProtocols, String securityManagerProtocols, String expected) throws Exception
    {
        ...
        try
        {
            ...
            if (!expected.equals(text))
                new Exception("doTestExternalDTD").printStackTrace();
            Assert.assertEquals(expected, text);
        }
        catch (SAXParseException e)
        {
            Assert.assertTrue(e.getLocalizedMessage().indexOf("Element type \"foo\" must be declared.") > -1);
        }
        catch (Exception e)
        {
            e.printStackTrace();
